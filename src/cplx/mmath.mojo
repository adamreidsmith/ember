...
fn kron_sequential[type: DType, a_rows: Int, a_cols: Int, b_rows: Int, b_cols: Int](
    a: CMatrix[type, a_rows, a_cols],
    b: CMatrix[type, b_rows, b_cols],
) -> CMatrix[type, a_rows * b_rows, a_cols * b_cols]:
    '''Implements the Kronecker product using the naive algorithm.'''
    var result = CMatrix[type, a_rows * b_rows, a_cols * b_cols](fill_zeros=False)
    var block: CMatrix[type, b.rows, b.cols]
...
fn swap_rows[type: DType, rows: Int, cols: Int](
    A: CMatrix[type, rows, cols], 
    r1: Int, 
    r2: Int
) raises -> CMatrix[type, rows, cols]:
...
fn swap_vals_inplace[type: DType, rows: Int, cols: Int](
    inout A: CMatrix[type, rows, cols], 
    r1: Int, 
    c1: Int, 
    r2: Int, 
    c2: Int
...
fn augmented_ref[type: DType, rows: Int, A_cols: Int, B_cols: Int, tol: Scalar[type] = 1e-15](
    A: CMatrix[type, rows, A_cols], B: CMatrix[type, rows, B_cols]
) raises -> CMatrix[type, rows, A_cols + B_cols]:
    '''Computes the row echelon form of the augmented matrix [A|B].'''
    var Aaug: CMatrix[type, rows, A_cols + B_cols] = hstack(A, B)
...
fn solve[type: DType, size: Int, B_cols: Int, tol: Scalar[type] = 1e-15](
    A: CMatrix[type, size, size], B: CMatrix[type, size, B_cols]
) raises -> CMatrix[type, size, B_cols]:
    '''Solves a linear system of equations Ax=B via Gaussian elimination.'''
    var aug_ref: CMatrix[type, size, size + B_cols] = augmented_ref[tol=tol](A, B)
    ...
    # Back substitution
    var X: CMatrix[type, size, B_cols] = B.zeros_like()
...
fn one_norm[type: DType, rows: Int, cols: Int](A: CMatrix[type, rows, cols]) -> SIMD[type, 1]:
    '''Computes the 1-norm of a matrix, i.e. the maximum absolute column sum.'''
    var col_sums: CMatrix[type, 1, cols] = CMatrix[type, 1, cols](fill_zeros=False)
    @parameter
...
fn _int_matrix_power[type: DType, size: Int](owned A: CMatrix[type, size, size], n: Int) -> CMatrix[type, size, size]:
...
fn matrix_power[type: DType, size: Int](A: CMatrix[type, size, size], n: Int) raises -> CMatrix[type, size, size]:
...
fn mmax[type: DType, rows: Int, cols: Int](A: CMatrix[type, rows, cols]) -> ComplexScalar[type]:
...
fn mmin[type: DType, rows: Int, cols: Int](A: CMatrix[type, rows, cols]) -> ComplexScalar[type]:
...
fn hstack[type: DType, rows: Int, A_cols: Int, B_cols: Int](
    A: CMatrix[type, rows, A_cols], B: CMatrix[type, rows, B_cols]
) -> CMatrix[type, rows, A_cols + B_cols]:
    '''Stack two matrices horizontally and return the result.'''
    var result = CMatrix[type, rows, A_cols + B_cols](fill_zeros=False)
    for r in range(rows):
...
fn vstack[type: DType, A_rows: Int, B_rows: Int, cols: Int](
    A: CMatrix[type, A_rows, cols], B: CMatrix[type, B_rows, cols]
) -> CMatrix[type, A_rows + B_rows, cols]:
    '''Stack two matrices vertically and return the result.'''
    var result = CMatrix[type, A_rows + B_rows, cols](fill_zeros=False)
    for c in range(cols):
        # TODO: Change to memcpy for improved efficiency
...
fn _expm_pade[type: DType, size: Int, m: Int](A: CMatrix[type, size, size]) raises -> CMatrix[type, size, size]:
    # alias b: List[Int, True] = b_d[(m - 3) // 2]  # This produces a segmentation fault...
    var b: List[Int, True] = b_d[(m - 3) // 2]

    var U: CMatrix[type, size, size] = A.eye_like() * ComplexScalar[type](b[1])
    var V: CMatrix[type, size, size] = A.eye_like() * ComplexScalar[type](b[0])
    var A2 = A @ A
    var A2n = A.eye_like()
...
fn _expm_ss[type: DType, size: Int](A: CMatrix[type, size, size], norm: SIMD[type, 1]) raises -> CMatrix[type, size, size]:
    # alias b = b_d[4]  # This produces a segmentation fault...
    var b = b_d[4]
    alias inv_log_2: SIMD[type, 1] = log(2.0).cast[type]()
    
    var Ac: CMatrix[type, size, size] = A
    var s: Int = max(0, int(ceil(log(norm / theta13) * inv_log_2)))
    if s > 0:
        Ac /= 2**s

    var I: CMatrix[type, size, size] = Ac.eye_like()
    var A2: CMatrix[type, size, size] = Ac @ Ac
    var A4: CMatrix[type, size, size] = A2 @ A2
    var A6: CMatrix[type, size, size] = A2 @ A4
    var U: CMatrix[type, size, size] = Ac @ (A6 @ (b[13] * A6 + b[11] * A4 + b[9] * A2) + b[7] * A6 + b[5] * A4 + b[3] * A2 + b[1] * I)
    var V: CMatrix[type, size, size] = A6 @ (b[12] * A6 + b[10] * A4 + b[8] * A2) + b[6] * A6 + b[4] * A4 + b[2] * A2 + b[0] * I
    var r13: CMatrix[type, size, size] = solve(V - U, V + U)
    # var r13: CMatrix[type, size, size] = (V - U).inv() @ (V + U)
    return matrix_power(r13, 2**s)


fn expm[type: DType, size: Int](A: CMatrix[type, size, size]) raises -> CMatrix[type, size, size]:
