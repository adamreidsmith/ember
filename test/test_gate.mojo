...
def test_unparameterized_multi_qubit_gates():
    ...
    gate = CX[type](0, 3)
    _assert_matrix_equal(gate.matrix, x, 'CX')
    assert_equal(gate.name, 'CX', 'CX')
    assert_equal(gate.n_qubits, 2, 'CX')
    assert_equal(len(gate.qubits), 1, 'CX')
    assert_equal(gate.qubits[0], 3, 'CX')
    assert_equal(len(gate.params), 0, 'CX')
    assert_equal(len(gate.controls), 1, 'CX')
    assert_equal(gate.controls[0], 0, 'CX')
    assert_equal(len(gate._measure_targs), 0, 'CX')
    assert_equal(len(gate.classical_controls), 0, 'CX')
    assert_equal(gate._is_measure, False, 'CX')
    # CCX
    gate = CCX[type](0, 1, 3)
    _assert_matrix_equal(gate.matrix, x, 'CCX')
    assert_equal(gate.name, 'CCX', 'CCX')
    assert_equal(gate.n_qubits, 3, 'CCX')
    assert_equal(len(gate.qubits), 1, 'CCX')
    assert_equal(gate.qubits[0], 3, 'CCX')
    assert_equal(len(gate.params), 0, 'CCX')
    assert_equal(len(gate.controls), 2, 'CCX')
    assert_equal(gate.controls[0], 0, 'CCX')
    assert_equal(gate.controls[1], 1, 'CCX')
    assert_equal(len(gate._measure_targs), 0, 'CCX')
    assert_equal(len(gate.classical_controls), 0, 'CCX')
    assert_equal(gate._is_measure, False, 'CCX')
