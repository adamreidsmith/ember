...
fn _assert_almost_equal[type: DType, size: Int](lhs: SIMD[type, size], rhs: SIMD[type, size], msg: String = '', *, atol: SIMD[type, 1] = 1e-10, rtol: SIMD[type, 1] = 0) raises:
...
fn _assert_matrix_almost_equal[type: DType](lhs: CMatrix[type], rhs: CMatrix[type], msg: String = '', *, atol: SIMD[type, 1] = 1e-10, rtol: SIMD[type, 1] = 0) raises:
    assert_equal(lhs.rows, rhs.rows, msg + ' rows')
    assert_equal(lhs.cols, rhs.cols, msg + ' cols')
    for r in range(lhs.rows):
        for c in range(lhs.cols):
...
fn _assert_matrix_almost_equal[type: DType](lhs: CMatrix[type], rhs: ComplexScalar[type], msg: String = '', *, atol: SIMD[type, 1] = 1e-10, rtol: SIMD[type, 1] = 0) raises:
    for r in range(lhs.rows):
        for c in range(lhs.cols):
            if not isnan(lhs[r, c].re) or not isnan(rhs.re):
